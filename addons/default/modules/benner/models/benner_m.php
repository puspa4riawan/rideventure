 ...
 */
class Benner_m extends MY_Model
{
	/* general */
	...
	/* articles */
    public function count_benner($params = array()) {

		if ( ! empty($params['keywords'])) {
			$this->db->like('benner.title', trim($params['keywords']));
		}

		$action = true;
		if(isset($params['date_start']) && isset($params['date_end'])){
            $this->db->where('benner.created BETWEEN "'.$params['date_start'].'" AND "'.$params['date_end'].'"', '', FALSE);
            $action = false;
        }

        if ($action) {
            if (isset($params['date_start'])) {
                $this->db->where('benner.created >=', $params['date_start']);
            }

            if (isset($params['date_end'])) {
                $this->db->where('benner.created <=', $params['date_end']);
            }
        }
		...
		if ( ! empty($params['status'])) {
			...
			if ($params['status'] != 'all') {
				...
				$this->db->where('benner.status', $params['status']);
			}
		} else {

			$this->db->where('benner.status', 'live');
		}
		...
		return $this->db->count_all_results('benner');
	}

	public function get_many_articles($params = array()) {
		if ( ! empty($params['keywords'])) {
			$this->db
				->like('article.title', trim($params['keywords']));
		}

		$action = true;
		if(isset($params['date_start']) && isset($params['date_end'])){
            $this->db->where('article.created BETWEEN "'.$params['date_start'].'" AND "'.$params['date_end'].'"', '', FALSE);
            $action = false;
        }

        if ($action) {
            if (isset($params['date_start'])) {
                $this->db->where('article.created >=', $params['date_start']);
            }

            if (isset($params['date_end'])) {
                $this->db->where('article.created <=', $params['date_end']);
            }
        }
		...
		if ( ! empty($params['status'])) {
			...
			$this->db->where('article.status !=', 'all');
		}

		if (array_key_exists('featured', $params)) {
			if (!empty($params['featured'])) {
				$this->db->where('article.featured', $params['featured']);
			}
		}
		...
		if ( ! empty($params['categories'])) {
			$this->db
				->like('article.kidsage_id', trim($params['categories']));
		}		
		...
		if (isset($params['order']) && is_array($params['order'])) {
			$this->db->order_by('article.'.$params['order'][0], $params['order'][1]);
