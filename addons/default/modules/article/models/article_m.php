...
class Article_m extends MY_Model
{
	...
	public function get_many_category($params = array()) {
		...
		if (!empty($params['milestone'])) {
			// $this->db
			// 	->where_in('kidsage_id', $params['milestone']);

			foreach($params['milestone'] as $key => $milestone) {
                if ($key == 0) {
                    $this->db->where('FIND_IN_SET(\''.$milestone.'\', kidsage_id)');
                } else {
                    $this->db->or_where('FIND_IN_SET(\''.$milestone.'\', kidsage_id)');
				}
			}
		}
	...
	/* articles */
    public function count_articles($params = array()) {

		if ( ! empty($params['keywords'])) {
			$this->db->like('article.title', trim($params['keywords']));
		}
		...
		if ( ! empty($params['categories'])) {
			...
			$this->db
				->like('category.name', trim($params['categories']));
		}
