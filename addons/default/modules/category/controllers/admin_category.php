...
class Admin_category extends Admin_Controller {
	...
	protected $forms_validation = array(
    	...
    	'slug'	=> array(
    		'field'	=> 'slug',
    		'label'	=> 'lang:category:slug',
    		'rules'	=> 'trim|required'
    	),

    	'status' => array(
			'field' => 'status',
			'label' => 'lang:category:status_label',
			'rules' => 'trim|alpha'
		),
    ...
    public function __construct() {
		...
		$this->lang->load(array('category','general'));

		

		$_template = array(
			'no-image'			=>'category without image',
			'thumb' 			=>'category with thumbnail',
			'2-tile' 			=>'category with merge tile',
			'gallery' 			=>'category with gallery',
	...
	public function index() {
		...
		//set filternya
		$this->category_m->set_filter($base_where);

		$total_rows = $this->category_m->count_data('category');

		
		$pagination = create_pagination(ADMIN_URL.'/category/'.$this->u_admin.'/index', $total_rows, Settings::get('records_per_page'),5);

		//set filternya sebelum select semua data

		$this->category_m->set_filter($base_where);
		$data_ 		= $this->category_m
						   ->limit($pagination['limit'], $pagination['offset'])
						   ->order_by('id', 'DESC')
						   ->get_all_data('category');
	...
	public function create() {
		...
		if ($this->form_validation->run()) {
			if ($this->input->post('status') == 'live') {
				role_or_die('category/admin_category', 'put_live');
				$hash = "";
			}
			...
			if ($id = $this->category_m->insert_data('category', $extra)) {
				if($_FILES['thumb']['name']!=''){
					...
					if($this->upload->do_upload('thumb')){
						...
						$new_data = array(
	                		...
	                		'path'		=> 'uploads/default/files/category/'.$id.'/',
	                		'full_path'	=> 'uploads/default/files/category/'.$id.'/'.$data['file_name'],
	                	);

	                	$this->category_m->update_data('category', $new_data, 'id', $id);
			...
			($this->input->post('btnAction') == 'save_exit') ? redirect(ADMIN_URL.'/category/'.$this->u_admin) : redirect(ADMIN_URL.'/category/'.$this->u_admin.'/edit/'.$id);
       	...
       	$this->template
			->title($this->module_details['name'], lang('category:title'))
	...
	public function edit($id = 0) {
		$id or redirect(ADMIN_URL.'/category/'.$this->u_admin);
		$data_ = $this->category_m->get_single_data('id', $id, 'category');
		// $data_usr = $this->ion_auth->get_user($data_->authors_id);

		if(! ($data_)) {
			redirect(ADMIN_URL.'/category/'.$this->u_admin);
		}
		...
		if ($this->form_validation->run()) {
			...
			if ($this->category_m->update_data('category', $extra, 'id', $id)) {
				if($_FILES['thumb']['name']!=''){
					$this->upload->initialize($this->set_upload_options($_FILES['thumb'], $id));
					if($this->upload->do_upload('thumb')){
						$data = $this->upload->data();
				...
				if ($_FILES['thumb_mobile']['name'] != '') {
					...
					if ($this->upload->do_upload('thumb_mobile')) {
						$data = $this->upload->data();
						$new_data = array('filename_mobile' => $data['file_name']);
	                	$this->article_m->update_data('category', $new_data, 'id', $id);
					} else {
						$errors = $this->upload->display_errors();
						$this->session->set_flashdata('error', $errors);
					}
				...
				$this->session->set_flashdata(array('success' => sprintf(lang('category:edit_success'), $this->input->post('name'))));
                $this->session->unset_userdata('upload');
			}
			else
			{
	...
	public function delete($id = 0) {
		role_or_die('category_m', 'delete_live');
        ...
        if ( ! empty($ids)) {
			foreach ($ids as $id) {
				if ($post = $this->category_m->get_single_data('id', $id, 'category')) {
					$this->category_m->delete_data('category', 'id', $id);
                    $this->maxcache->delete('category_m');
	...
	public function publish($id = 0) {
		role_or_die('category_m', 'put_live');
		...
		if ( ! empty($ids)) {
			foreach ($ids as $id) {
				if ($post = $this->category_m->get_single_data('id', $id, 'category')) {
					$this->category_m->publish_data('category', 'id', $id);
					$this->maxcache->delete('category_m');
					$post_titles[] = $post->name;
				}
			}
