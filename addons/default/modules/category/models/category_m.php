...
class Category_m extends MY_Model
{
	...
	/* comments */
   	...
   	public function set_filter($params = array()){
   		if ( ! empty($params['keywords'])) {
			...
			$this->db
				->like('category.name', trim($params['keywords']));
		}

		$action = true;
		if(isset($params['date_start']) && isset($params['date_end'])){
            $this->db->where('category.created BETWEEN "'.$params['date_start'].'" AND "'.$params['date_end'].'"', '', FALSE);
            $action = false;
        }

        if ($action) {
            if (isset($params['date_start'])) {
                $this->db->where('category.created >=', $params['date_start']);
            }

            if (isset($params['date_end'])) {
                $this->db->where('category.created <=', $params['date_end']);
            }
        }
		...
		if ( ! empty($params['status'])) {
			...
			if ($params['status'] != 'all') {
				...
				$this->db->where('category.status', $params['status']);
			}
		} else {

			$this->db->where('category.status', 'live');
		}
	...
	public function check_exists($table, $params=array()) {
		if ( ! empty($params['name'])) {
			$this->db->where($table.'.name', $params['name']);
		}
