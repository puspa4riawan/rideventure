	...
	/* comments */
   	...
   	public function set_filter($params = array()){
   		if ( ! empty($params['keywords'])) {
			...
			$this->db
				->like('survey.question', trim($params['keywords']));
		}

		$action = true;
		if(isset($params['date_start']) && isset($params['date_end'])){
            $this->db->where('survey.created BETWEEN "'.$params['date_start'].'" AND "'.$params['date_end'].'"', '', FALSE);
            $action = false;
        }

        if ($action) {
            if (isset($params['date_start'])) {
                $this->db->where('survey.created >=', $params['date_start']);
            }

            if (isset($params['date_end'])) {
                $this->db->where('survey.created <=', $params['date_end']);
            }
        }
		...
		if ( ! empty($params['status'])) {
			...
			if ($params['status'] != 'all') {
				...
				$this->db->where('survey.status', $params['status']);
			}
		} else {

			$this->db->where('survey.status', 'live');
		}
