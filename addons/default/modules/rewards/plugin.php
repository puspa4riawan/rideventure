...
class Plugin_article extends Plugin
{
	...
	public function milestone_section_on_homepage()
	{
		$this->load->model('article_m');
    	...
    	$data_articles = array();
    	$data_articles_main = array();

    	foreach ($milestone_data_id as $key => $mid) {
			$parameter = array(
				'status'    => 'live',
				'order_by'  => array('article_id', 'DESC'),
				'milestone' => $mid
			...
			$articles_feat = $this->article_m->getDataArticle($parameter, $pagination)->result();

			foreach ($articles as $article) {
				$mids = explode(',', $article->kidsage_id);
				$article->milestone = $this->article_m->get_milestone_for_article($mids);
				$article->images = $this->article_m->get_images_for_article($article->article_id);
			}

			foreach ($articles_feat as $article) {
				$mids = explode(',', $article->kidsage_id);
				$article->milestone = $this->article_m->get_milestone_for_article($mids);
				$article->images = $this->article_m->get_images_for_article($article->article_id);
			}

			$data_articles[$key] = $articles;

			if (!$articles_feat && $articles) {
				$articles_feat = $articles;
			}

			$data_articles_main[$key] = $articles_feat;
		}

		return $this->load->view('article/plugin/milestone_homepage', array(
			'articles'  => $data_articles,
			'articles_main'  => $data_articles_main,
			'milestone' => $milestone_data_id,
			'timezone'  => isset($_COOKIE['tzu']) ? $this->security->xss_clean($_COOKIE['tzu']) : ''
		), true);
	}

	public function render_milestone_tools()
	{
		$this->load->model('article_m');

		$type = $this->attribute('type');
		$milestone_data = $this->attribute('milestone');
		$milestone_data = explode(',', $milestone_data);
		$milestone = array();
		...
		return $this->load->view('article/plugin/single_milestone_tool', array(
			'type'  => $type,
			'tools' => $tools && isset($tools[1]) ? $tools[1] : array()
		), true);
	}
	...
	public function get_ads()
	{
		...
		$milestone = $this->attribute('milestone') ? $this->attribute('milestone') : '';
		$parameter = array(
			'status' => 'live',
			'limit'  => 1
		);
		...
		if (!empty($milestone)) {
			$milestone_data = explode(',', $milestone);
			...
			$parameter['milestone'] = $milestone_param;
		}

		$this->db->reset_query();
		$ads = $this->article_m->get_many_articles_ads($parameter);

		return $this->load->view('article/plugin/render_ads', array('ads' => (array) $ads), true);
	}

	public function tools_ikutserta_home()
	{
		$this->load->model('article_m');
    	...
    	$tools = $this->article_m->get_tools_ikutserta_list($tools_parameter);

		return $this->load->view('article/plugin/tools_ikutserta', array('tools' => $tools), true);
	}

	public function tools_ikutserta_milestone()
	{
		$this->load->model('article_m');
		$milestone = $this->attribute('milestone');
