 ...
 */
class Admin_rewards extends Admin_Controller {

	var $u_admin 		= 'admin_rewards';
	protected $section 	= 'rewards';

	protected $forms_validation = array(
    	'title'	=> array(
    		'field'	=> 'title',
    		'label'	=> 'lang:rewards:title',
    		'rules'	=> 'trim|required|xss_clean|callback__check_title'
    	),

    	'desc' => array(
			'field'	=> 'description',
			'label'	=> 'lang:rewards:desc_label',
			'rules'	=> 'trim'
		),

    	'slug'	=> array(
    		'field'	=> 'slug',
    		'label'	=> 'lang:rewards:slug',
    		'rules'	=> 'trim|required|xss_clean|callback__check_slug'
    	),

    	'status' => array(
			'field' => 'status',
			'label' => 'lang:rewards:status_label',
			'rules' => 'trim|alpha'
		),
    ...
    public function __construct() {
		...
		$this->load->model(array('rewards_m'));
		
		$this->load->libraries(array('upload'));
		$this->lang->load(array('rewards','general'));

		

		$_template = array(
			'no-image'			=>'Rewards without image',
			'thumb' 			=>'Rewards with thumbnail',
			'2-tile' 			=>'Rewards with merge tile',
			'gallery' 			=>'Rewards with gallery',
	...
	public function index() {
		...
		//set filternya
		$this->rewards_m->set_filter($base_where);

		$total_rows = $this->rewards_m->count_data('rewards');

		
		$pagination = create_pagination(ADMIN_URL.'/rewards/'.$this->u_admin.'/index', $total_rows, Settings::get('records_per_page'),5);

		//set filternya sebelum select semua data

		$this->rewards_m->set_filter($base_where);
		$data_ 		= $this->rewards_m
						   ->limit($pagination['limit'], $pagination['offset'])
						   ->order_by('id', 'DESC')
						   ->get_all_data('rewards');
		...
		$this->template
			...
			->set_partial('filters', 'admin/rewards/partials/filters')
			->set('pagination', $pagination)
			->set('data_', $data_)
			->set('u_admin', $this->u_admin);

			$this->input->is_ajax_request()
			? $this->template->build('admin/rewards/tables/table')
			: $this->template->build('admin/rewards/index');
	}
	...
	public function create() {
		...
		if ($this->form_validation->run()) {
			if ($this->input->post('status') == 'live') {
				role_or_die('rewards/admin_rewards', 'put_live');
				$hash = "";
			}
			...
			$extra = array(
				...
				'show_percentage' 	=> $this->input->post('show_percentage'),
			...
			if ($id = $this->rewards_m->insert_data('rewards', $extra)) {
				if($_FILES['thumb']['name']!=''){
					...
					if($this->upload->do_upload('thumb')){
						...
						$new_data = array(
	                		...
	                		'path'		=> 'uploads/default/files/rewards/'.$id.'/',
	                		'full_path'	=> 'uploads/default/files/rewards/'.$id.'/'.$data['file_name'],
	                	);

	                	$this->rewards_m->update_data('rewards', $new_data, 'id', $id);
			...
			($this->input->post('btnAction') == 'save_exit') ? redirect(ADMIN_URL.'/rewards/'.$this->u_admin) : redirect(ADMIN_URL.'/rewards/'.$this->u_admin.'/edit/'.$id);
       	...
       	$this->template
			->title($this->module_details['name'], lang('rewards:title'))
	...
	public function edit($id = 0) {
		$id or redirect(ADMIN_URL.'/rewards/'.$this->u_admin);
		$data_ = $this->rewards_m->get_single_data('id', $id, 'rewards');
		// $data_usr = $this->ion_auth->get_user($data_->authors_id);

		if(! ($data_)) {
			redirect(ADMIN_URL.'/rewards/'.$this->u_admin);
		}
		//cek title
		
		$rewards_validation = array_merge($this->forms_validation, array(
			'title'	=> array(
	    		'field'	=> 'title',
	    		'label'	=> 'lang:rewards:title',
		...
		$rules 	= array_merge($this->forms_validation, $rewards_validation);

		$newtitle = $this->input->post('title');
		if($newtitle == $data_->title){
			unset($rules['title']);
		}

		$newslug = $this->input->post('slug');
		if($newslug == $data_->slug){
			unset($rules['slug']);
		}
		...
		if ($this->form_validation->run()) {
			...
			if ($this->rewards_m->update_data('rewards', $extra, 'id', $id)) {
				if($_FILES['thumb']['name']!=''){
					$this->upload->initialize($this->set_upload_options($_FILES['thumb'], $id));
					if($this->upload->do_upload('thumb')){
						$data = $this->upload->data();
				...
				if ($_FILES['thumb_mobile']['name'] != '') {
					...
					if ($this->upload->do_upload('thumb_mobile')) {
						$data = $this->upload->data();
						$new_data = array('filename_mobile' => $data['file_name']);
	                	$this->article_m->update_data('rewards', $new_data, 'id', $id);
					} else {
						$errors = $this->upload->display_errors();
						$this->session->set_flashdata('error', $errors);
					}
				...
				$this->session->set_flashdata(array('success' => sprintf(lang('rewards:edit_success'), $this->input->post('title'))));
                $this->session->unset_userdata('upload');
			}
			else
			{
				$this->session->set_flashdata('error', lang('rewards:edit_error'));
			}
	...
	public function delete($id = 0) {
		role_or_die('rewards_m', 'delete_live');
        ...
        if ( ! empty($ids)) {
			foreach ($ids as $id) {
				if ($post = $this->rewards_m->get_single_data('id', $id, 'rewards')) {
					$this->rewards_m->delete_data('rewards', 'id', $id);
                    $this->maxcache->delete('rewards_m');
	...
	public function publish($id = 0) {
		role_or_die('rewards_m', 'put_live');
		...
		if ( ! empty($ids)) {
			foreach ($ids as $id) {
				if ($post = $this->rewards_m->get_single_data('id', $id, 'rewards')) {
					$this->rewards_m->publish_data('rewards', 'id', $id);
					$this->maxcache->delete('rewards_m');
					$post_titles[] = $post->title;
				}
			}
	...
	public function _check_title($title, $id=0) {
		$this->form_validation->set_message('_check_title', sprintf(lang('rewards:already_exist_error'), lang('global:title')));

		return $this->rewards_m->check_exists('rewards', array('title'=>$title, 'id'=>$id));
	}

	public function _check_slug($slug, $id=0) {
		$this->form_validation->set_message('_check_slug', sprintf(lang('rewards:already_exist_error'), lang('global:slug')));

		return $this->rewards_m->check_exists('rewards', array('slug'=>$slug, 'id'=>$id));
	}
	...
	private function set_upload_options($file, $id) {
		$new_path = '/uploads/default/files/rewards/'.$id.'/';
