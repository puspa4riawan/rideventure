...
if ( ! file_exists('system/cms/config/database.php'))
{
	...
	{
		header('Status: 404');
		exit('MaxCMS is missing system/cms/config/database.php and cannot find the installer folder. Does your server have permission to access these files?');
	}
...
if(preg_match('/(?i)msie [5-8]/',$_SERVER['HTTP_USER_AGENT'])){
    // if IE<=8
    echo 'Browser anda sudah kadaluarsa silakan gunakan browser yang lebih up to date, untuk menampilkan halaman ini';
    exit;
}
 ...
 *---------------------------------------------------------------
 *
 * Should MaxCMS run as a demo, meaning no destructive actions
 * can be taken such as removing admins or changing passwords?
 *
 */

    define('MAX_DEMO', (file_exists(FCPATH.'DEMO')));
