		...
		<tbody>
			...
			<tr>
				...
				<?php if ($widget->enabled == '1'): ?>
					<?php echo anchor(ADMIN_URL.'/addons/widgets/disable/' . $widget->id, lang('buttons:disable'), 'class="button disable"') ?>
				<?php else: ?>
					<?php echo anchor(ADMIN_URL.'/addons/widgets/enable/' . $widget->id, lang('buttons:enable'), 'class="button enable"') ?>
				<?php endif ?>
				</td>
			</tr>
			<?php endforeach ?>
