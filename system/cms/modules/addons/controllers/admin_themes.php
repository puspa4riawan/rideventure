 ...
 *
 * @package 	MaxCMS\Core\Modules\Addons\Controllers
...
class Admin_themes extends Admin_Controller
{
	...
	public function options($slug = '')
	{
		...
		if ($this->input->post('btnAction') === 're-index')
		{
			...
			if ($this->theme_m->get_all())
			{
				...
				$this->maxcache->delete_all('theme_m');

				redirect(ADMIN_URL.'/addons/themes/options/'.$slug);
			}
		}
		...
		if ($all_options)
		{
			...
			foreach ($all_options as $option)
			{
				$this->validation_rules[] = array(
					...
					'rules' => 'trim'.($option->is_required ? '|required' : ''),
				);
	...
	public function set_default()
	{
		...
		{
			redirect(ADMIN_URL.'/addons/themes/admin_themes');
		}

		redirect(ADMIN_URL.'/addons/themes');
	}
