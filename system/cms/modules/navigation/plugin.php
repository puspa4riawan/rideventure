...
class Plugin_Navigation extends Plugin
{
	...
	public function links()
	{
		...
		$links = $this->maxcache->model('navigation_m', 'get_link_tree', $params, config_item('navigation_cache'));

		return $this->_build_links($links, $this->content());
	}

	public function linkstree()
	{
		$group         = $this->attribute('group');
		$group_segment = $this->attribute('group_segment');
	...
	public function getmenu()
	{
		...
		if($group=='header') {
			$view = 'navigation/plugin/main-menu';
		} elseif ($group=='footer') {
			$view = 'navigation/plugin/footer-menu';
		} elseif($group == 'newheader'){
			$view = 'navigation/plugin/newheader';
		} else{
			$view = 'navigation/plugin/default';
		}

		return $this->load->view($view,array('links' => $links), true);
	}
