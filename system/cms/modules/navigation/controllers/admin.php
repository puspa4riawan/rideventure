...
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		/* tools */
		$this->template->tools_select 	= array(
			'ovulation-calender'									=>array(
																		'label'		=>'Ovulation calender',
																		'uri'		=>'ovulation-calender',
																	),
			'pregnancy-duedate'										=>array(
																		'label'		=>'Pregnancy duedate',
																		'uri'		=>'pregnancy-duedate',
																	),
			'smart-strength-finder-tools'							=>array(
																		'label'		=>'Smart strength finder',
																		'uri'		=>'smart-strength-finder-tools',
																	),
			'smart-consultation'									=>array(
																		'label'		=>'Smart consultation',
																		'uri'		=>'smart-consultation',
																	),
			'smart-strength-finder/21-days-smart-stimulations'		=>array(
																		'label'		=>'Smart stimulation',
																		'uri'		=>'smart-strength-finder/21-days-smart-stimulations',
																	),
			'smart-school'											=>array(
																		'label'		=>'Smart school',
																		'uri'		=>'smart-school',
																	),
			'kalkulator-afs'										=>array(
																		'label'		=>'Kalkulator AFS',
																		'uri'		=>'kalkulator-afs',
																	),
			'vaksin-tools'										=>array(
																		'label'		=>'Imunisasi',
		...
		/* milestone */
		$ci = get_instance();
		$ci->load->config('article/config');
		$milestone_select = $milestonelist_select = array();

		foreach($ci->config->item('article_route_milestone') as $key=>$milestone) {
			$milestone_select[$key] = ucfirst($key);
		};
		$this->template->milestone_select  = $milestone_select;

		foreach($ci->config->item('article_route_milestone') as $key=>$milestone) {
			$milestonelist_select[$key.'/list'] = ucfirst($key);
		};
		$this->template->milestonelist_select  = $milestonelist_select;
	...
	public function order()
	{
		...
		if (is_array($order))
		{
			...
			$this->maxcache->delete_all('navigation_m');
	...
	public function create($group_id = '')
	{
		...
		if ($this->form_validation->run())
		{
			...
			if(in_array($input['link_type'], array('milestone', 'milestonelist', 'tools', 'article'))) {
				$input['uri'] = $input['uri_'.$input['link_type']];
				$input['url'] = '';
			}elseif ($input['link_type']=="divider") {
				$input['url'] = '#';
			}
	...
	public function edit($id = 0)
	{
		...
		if($link->link_type=='article') {
			...
			$dataArticle = $this->article_m
							  ->where(array('article.slug' => $link->uri))
							  ->get_single_articles();
			$link->article_title = $dataArticle->title;
		} else {
			$link->article_title = '';
		}
	...
	public function getarticle()
	{
		...
		if($this->input->is_ajax_request()){
			...
			$this->form_validation->set_rules($validationArticle);
			// if($this->form_validation->run()) {
			$post = $this->input->post();
			if(array_key_exists('title_article', $post)) {
				if(!empty($post['title_article'])) {
					$title 		= $this->input->post('title_article');
					$article 	= $this->navigation_m->get_article($title);
			   		$article 	= array_for_select($article, 'slug', 'title');
