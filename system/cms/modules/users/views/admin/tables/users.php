<fieldset>
	Total User : <?php echo $total_rows; ?>
</fieldset>
<?php if (!empty($users)): ?>
	<table border="0" class="table-list" cellpadding="0" cellspacing="0">
		<thead>
			<tr>
				...
				<th>No</th>
				<th width="10%"><?php echo lang('user:name_label');?></th>
				...
				<th class="collapse">Blogger</th>
				<th class="collapse">Expert</th>
				<th width="200"></th>
			</tr>
		</thead>
		<tfoot>
			...
			<?php
				...
				<tr>
					...
					<?php if ($link_profiles) : ?>
						<?php echo anchor(ADMIN_URL.'/users/preview/' . $member->id, $member->display_name, 'target="_blank" class="modal-large"') ?>
					<?php else: ?>
					...
					<td class="collapse"><?= (bool)$member->is_blogger ? 'Yes' : 'No'?></td>
					<td class="collapse"><?= (bool)$member->is_expert ? 'Yes' : 'No'?></td>
					<td class="actions">
						<?php echo anchor(ADMIN_URL.'/users/edit/' . $member->id, lang('global:edit'), array('class'=>'button edit')) ?>
						<?php echo anchor(ADMIN_URL.'/users/delete/' . $member->id, lang('global:delete'), array('class'=>'confirm button delete')) ?>
					</td>
				</tr>
			<?php endforeach ?>
		</tbody>
