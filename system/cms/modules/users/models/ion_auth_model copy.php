...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $group_id = null, $additional_data = array(), $group_name = false)
	{
		...
		$extra = array('user_id' => $id) + $additional_data;
...
/*	public function login($identity, $password, $remember=false)
	...
	public function login($identity, $password, $remember=false)
	{
		...
		$this->db->select('username, email, id, password, group_id,active, time_lock')
			->where(sprintf('(username = "%1$s" OR LCASE(email) = "%1$s")', $this->db->escape_str($identity)));
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($user->password === $password)
			{
				...
				//$this->db->update($this->tables['meta'],array('address'=>0));
				$this->db->update($this->tables['meta'],array('password_temp'=>0));
				return true;
			}
			else {
				$max_attempt_login = $this->config->item('max_attempt_login', 'ion_auth');
				$profile_attempt = $this->db->where(array('user_id'=>$user->id))->limit(1)->get($this->tables['meta']);
				if($profile_attempt->num_rows() == 1)
				{
					$profile_dat = $profile_attempt->row();
					if((intval($profile_dat->password_temp) >= $max_attempt_login) && $user->active == 1 )
					{
						if(empty($user->time_lock)) {
							$condition = array('active'=>0,'time_lock'=>date('Y-m-d H:i:s'));
						} else {
							$condition = array('active'=>0);
						}
						$this->db->where(array('id'=>$user->id));
						$this->db->update($this->tables['users'],$condition);
						$this->session->set_userdata('lockout_user','User has been lockout');

						$data = $this->db->select('*')
										 ->from('users')
										 ->where(array('email'=>$identity))->get()->row();
						if(!$data){
							$this->forgotten_password($identity);
							$data = $this->db->select('*')
											 ->from('users')
											 ->where(array('email'=>$identity))->get()->row();
						}
					...
					//else if(intval($profile_dat->address)<$max_attempt_login){
					else if(intval($profile_dat->password_temp)<$max_attempt_login){

						$this->db->where(array('user_id'=>$user->id));
						//$this->db->update($this->tables['meta'],array('address'=>intval($profile_dat->address)+1));
						$this->db->update($this->tables['meta'],array('password_temp'=>intval($profile_dat->password_temp)+1));

						if(intval($profile_dat->password_temp)+1==3){
							$this->forgotten_password($identity);
							$data = $this->db->select('*')
											 ->from('users')
											 ->where(array('email'=>$identity))->get()->row();
					...
					$this->login_log($user->username, $identity, 'Invalid Password');
				}
		...
		if($query->num_rows() == 1)
		{
			$this->login_log($query->row()->username, $identity, 'User Locked');
			$this->session->set_userdata('lockout_user','User has been lockout');
		}else{
			$this->login_log('-', $identity, 'User not exist');
		}
	...
	public function get_users($group = false, $limit = null, $offset = null)
	{
		...
		$this->db->select($this->tables['meta'].'.tw_id as tw_id');
		$this->db->select($this->tables['meta'].'.fb_id as fb_id');
		$this->db->select($this->tables['meta'].'.photo_profile as photo_profile');
		$this->db->select($this->tables['meta'].'.eways_id');
		$this->db->select($this->tables['meta'].'.total_point');
		$this->db->select($this->tables['meta'].'.is_expert');
