...
<div class="buttons">
	<?php 
		...
		{
			echo anchor(ADMIN_URL.'/pages/choose_type?modal=true&parent='.$page->id, lang('pages:create_label'), 'class="button modal"');
		}
		else
		{
			$type_id = $this->db->select('id')->limit(1)->get('page_types')->row()->id;
			echo anchor(ADMIN_URL.'/pages/create?parent='.$page->id.'&page_type='.$type_id, lang('pages:create_label'), 'class="button"');
		}

	?>
	<?php echo anchor(ADMIN_URL.'/pages/duplicate/'.$page->id, lang('pages:duplicate_label'), 'class="button"') ?>
	<?php echo anchor(ADMIN_URL.'/pages/edit/'.$page->id, lang('global:edit'), 'class="button"') ?>
	<?php echo anchor(ADMIN_URL.'/pages/delete/'.$page->id, lang('global:delete'), 'class="confirm button"') ?>
</div>