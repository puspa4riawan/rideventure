 ...
 * @package MaxCMS\Core\Modules\Pages\Models
 *
 */
class Page_m extends MY_Model
{
	...
	public function __construct() {
		...
		$this->_table_meta = 'pages_meta';
		$this->load->model(array('pages/page_type_m'));
	}
	...
	public function get_by_uri($uri, $is_request = false, $simple_return = false)
	{
		...
		if ($uri === null)
		{
			$page = $this->maxcache->model('page_m', 'get_home', array());
		}
		else
		{
			...
			{
				$page = $this->maxcache->model('page_m', 'get_page_simple', array($uri));
		...
		if ( ! $page->layout = $this->maxcache->model('page_type_m', 'get', array($page->type_id)))
		{
			// Some pillock deleted the page layout, use the default and pray to god they didnt delete that too
			$page->layout = $this->maxcache->model('page_type_m', 'get', array(1));
		}
		...
		if ($page->entry_id and $page->layout->stream_id)
		{
			...
			// Get Streams
			$stream = $this->maxcache->model('streams_m', 'get_stream', array($page->layout->stream_id));
	...
	*/
	...
	public function insert_page_meta($tabel, $data){
		$this->db->insert($tabel, $data);
		return $this->db->insert_id();
	}

	public function update_page_meta($tabel, $data, $where){
		return $this->db->update($tabel, $data, $where);
	}

	function delete_page_meta($tabel, $id=0){
		return $this->db->delete($tabel, array('id'=>$id));
	}

	function delete_pages_meta($tabel, $id=0){
		return $this->db->delete($tabel, array('page_id'=>$id));
	}

	public function get_page_meta($params=array()){
		$data_meta = array();
		$i = 0;
		
		if(isset($params['page_id'])){
			$this->db->where($this->_table_meta.'.page_id', $params['page_id']);
		}
		...
		$page_meta=$this->db->get_where($this->_table_meta)->result_array();
		foreach ($page_meta as $key => $item_meta) {
			$content 	= explode(',', $item_meta['content_id']);
			$fmeta		= array();
			
			$fdata = array(); // $this->qa_m->get_many_qa_array(array('active'=>'live'));
			if(count($fdata) > 0) {
				foreach ($fdata as $key => $fitem) {
					$fmeta[$i] = array(
						'qa_id'		=>$fitem['qa_id'],
						'question'	=>$fitem['question'],
						'answer'	=>$fitem['answer'],
						'checked'	=>(in_array($fitem['qa_id'], $content)) ? 1 : 0,
					);
					$i++;
				}
			}
			$data_meta[] = array(
				'id'				=>$item_meta['id'],
				'order'				=>$item_meta['order'],
				'item_title'		=>$item_meta['item_title'],
				'item_content'		=>$item_meta['item_content'],
				'page_id'			=>$item_meta['page_id'],
				'content'			=>$fmeta,
			);
		}
	...
	public function get_page_tab($tab=0, $get_data = true, $set_active='')
	{
		$pages = $this->db
			->select('pages.*, page_types.id as page_type_id, page_types.stream_id, page_types.body')
			->select('page_types.save_as_files, page_types.slug as page_type_slug, page_types.title as page_type_title, page_types.js as page_type_js, page_types.css as page_type_css')
			->join('page_types', 'page_types.id = pages.type_id', 'left')
			->where('pages.show_ontab', $tab)
			->order_by('`order`')
			->get($this->_table)
			->result();
			...
			if ($page and $page->type_id and $get_data) {
				...
				if ($stream) {
					...
					if (isset($ret['entries'][0])) {
						...
						$pages[$key] = array_merge((array) $pages[$key], (array) $ret['entries'][0]);
					}

					$this->page_type_m->get_page_type_files_for_page($page);
				}

				$page_content = $this->page_m->get_page_meta(array('page_id'=>$page->id));
				$pages[$key]['page_content'] = $page_content;

				if ($page->slug == $set_active) {
					$pages[$key]['as_active'] = 'active';
				} else {
					if ($key==0 and $set_active=='') {
						$pages[$key]['as_active'] = 'active';
					}
				}
			}
