...
class Admin_types extends Admin_Controller
{
		...
		if ($this->form_validation->run())
		{
			...
			if ($input['stream_id'] == 0)
			{
				...
				if ( ! $stream_slug and $this->db->table_exists($stream_slug))
				{
					...
					redirect(ADMIN_URL.'/pages/types/create');
				}
				else
				{
			...
			if ($id > 0)
			{
				...
				$this->maxcache->delete_all('page_type_m');
				$this->maxcache->delete_all('streams_m');
				...
				if ($this->input->post('stream_id') == 'new')
				{
					...
					redirect(ADMIN_URL.'/pages/types/fields/' . $id);
				}
			}
			else
			{
			...
			redirect(ADMIN_URL.'/pages/types');
		}
	...
	public function edit($id = 0)
	{
		...
		$data = new stdClass();
		empty($id) AND redirect(ADMIN_URL.'/pages/types');
		...
		if ($this->form_validation->run())
		{
			...
			$this->input->post('btnAction') == 'save_exit'
				? redirect(ADMIN_URL.'/pages/types')
				: redirect(ADMIN_URL.'/pages/types/edit/'.$id);
		}
	...
	public function fields()
	{
		...
		$extra = array(
			'add_uri'		=> ADMIN_URL.'/pages/types/fields/'.$page_type->id.'/new_field',
		...
		$extra['buttons'] = array(
			array(
				...
				'url'       => ADMIN_URL.'/pages/types/fields/'.$this->uri->segment(5).'/edit_field/-assign_id-'
			),
			array(
				'label'     => lang('global:delete'),
				'url'       => ADMIN_URL.'/pages/types/fields/'.$this->uri->segment(5).'/delete_field/-assign_id-',
				'confirm'   => true,
			)
		);
		...
		$this->streams->cp->assignments_table($stream->stream_slug, $stream->stream_namespace, Settings::get('records_per_page'), ADMIN_URL.'/pages/types/fields/'.$page_type->id, true, $extra);
	}
	...
	private function _new_field($stream, $page_type)
	{
		$extra = array(
			...
			'cancel_uri'			 => ADMIN_URL.'/pages/types/fields/'.$page_type->id,
			'allow_title_column_set' => true
		);

		$this->streams->cp->field_form($stream->stream_slug, $stream->stream_namespace, 'new', ADMIN_URL.'/pages/types/fields/'.$this->uri->segment(5), null, array(), true, $extra, array('chunks'));
	}
	...
	private function _edit_field($stream, $page_type)
	{
		...
		$this->streams->cp->field_form($stream->stream_slug, $stream->stream_namespace, 'edit', ADMIN_URL.'/pages/types/fields/'.$this->uri->segment(5), $this->uri->segment(7), array(), true, $extra, array('chunks'));
	}
	...
	private function _delete_field($stream)
	{
		...
		redirect(ADMIN_URL.'/pages/types/fields/'.$this->uri->segment(5));
	}
