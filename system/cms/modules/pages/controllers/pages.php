...
class Pages extends Public_Controller
{
	...
	public function _page($url_segments)
	{
		...
		// results immediately.
		if (ENVIRONMENT == MAX_DEVELOPMENT)
		{
		...
		if ( ! $page or ($page->status == 'draft' and ! $this->permission_m->has_role(array('put_live', 'edit_live'))))
		{
			...
			if ( ! ($page = $this->maxcache->model('page_m', 'get_by_uri', array('404'))) )
			{
		...
		if ($url_segments = explode('/', $page->base_uri) and count($url_segments) > 1)
		{
			...
			if ( ! $parents = $this->maxcache->get('page_m/'.md5(implode('/', $url_segments))))
			{
				...
				foreach ($url_segments as $segment)
				{
					...
					$parents[] = $this->maxcache->model('page_m', 'get_by_uri', array($breadcrumb_segments, true, true));
				}

				// Cache for next time
				$this->maxcache->write($parents, 'page_m/'.md5(implode('/', $url_segments)));
			}
		...
		$temp = array();
		$temp = $this->page_m->get_descendant_ids($page->id);// var_dump($theme); die();

		$stream_childrend = array();
		$b = $html;
		foreach ($temp as $temps) {
			$a = $this->page_m->get($temps);

			//var_dump($a);

			$stream = $this->streams_m->get_stream($a->stream_id);
			$stream_childrend[]	= $stream;

			$b = $this->parser->parse_string($b, $a, true, false, array(
				'stream' => $stream->stream_slug,
				'namespace' => $stream->stream_namespace,
				'id_name' => 'entry_id'
			));
			$page = array_merge((array)$a,(array)$page);
		}
		/*$page_tmp = new stdClass;
		foreach($page as $idsx=>$valsx)
		{
			$page_tmp->{$idsz} =$valsx;
		}*/
		$page = (object) $page;
		...
		$this->template
					->build($view, array('page' => $page)+(array)$page, false, false, true, $template);
	}
	...
	public function _rss($url_segments)
	{
		...
		$page = $this->maxcache->model('page_m', 'get_by_uri', array($url_segments, true));
		...
		// Hit the query through MaxCache.
		$children = $this->maxcache->model('page_m', 'get_many_by', array($query_options));
