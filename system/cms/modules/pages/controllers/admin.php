 ...
 * @package 	MaxCMS\Core\Modules\Pages\Controllers
 */
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$qa_list = array(); // $this->qa_m->get_many_qa_array(array('status'=>'live'));
		...
		$this->template->set('qa_list', $qa_list);
	}
	...
	public function choose_type()
	{
        ...
        if (count($all) === 1) 
        {
            redirect(ADMIN_URL.'/pages/create?page_type='.$all[0]->id.$parent);
        }
        ...
        if ($this->input->get('modal') === 'true') 
        {
    		...
    		foreach ($all as $pt)
    		{
    			$html .= '<li><a href="'.site_url(ADMIN_URL.'/pages/create?page_type='.$pt->id.$parent).'"><strong>'.$pt->title.'</strong>';
	...
	public function order()
	{
		...
		if (is_array($order))
		{
			...
			$this->maxcache->delete_all('page_m');
	...
	public function create()
	{
        ...
        if ( ! $page_type_id) 
        {
            redirect(ADMIN_URL.'/pages/choose_type');
        }
		...
		if ($this->form_validation->run())
		{
			...
			if ($id = $this->page_m->create($input, $stream))
			{
				...
				if (array_key_exists('tabcontent_id',$input)) {
					if (count($input['tabcontent_id'])>0 && $input['tabcontent_id']) {
						for($i=0; $i<count($input['tabcontent_id']); $i++) {
							if (!empty($input['tabcontent_id'][$i])) {
								$page_meta = array(
									'page_id'			=> $id,
									'bg_color'			=> $input['label_bgcolor'][$i],
									'item_title'		=> $input['item_title'][$i],
									'item_content'		=> $input['item_content'][$i],
									'content_id'		=> implode(',', $input['content_id'][$i]),
									'created_on'		=> now(),
									'updated_on'		=> now(),
									'order'				=> (int)$input['order'][$i],
								);

								$meta_id = $this->page_m->insert_page_meta($this->_table_meta,$page_meta);
							}
						}
					}
				}
		...
		$this->template
			...
			->append_css('module::flexible-form.css')
			->build('admin/form');
	}
	...
	public function edit($id = 0)
	{
		...
		$id or redirect(ADMIN_URL.'/pages');
		...
		$tabcontent = array();
		$tabcontent['content'] = $this->page_m->get_page_meta(array('page_id'=>$id));
		...
		if ($this->form_validation->run())
		{
			...
			if ($this->page_m->edit($id, $input, $stream, $page->entry_id))
			{
				...
				if (array_key_exists('tabcontent_id',$input)) {
					if (count($input['tabcontent_id'])>0) {
						for($i=0; $i<count($input['tabcontent_id']); $i++) {
							if (array_key_exists($i,$input['tabcontent_id'])) {
								if (!empty($input['flexible_id'][$i])) {
									$page_meta = array(
										'item_title'		=> $input['item_title'][$i],
										'item_content'		=> $input['item_content'][$i],
										'content_id'		=> (array_key_exists('content_id', $input)) ? implode(',', $input['content_id'][$i]) : '',
										'updated_on'		=> now(),
										'order'				=> (int)$input['order'][$i],
									);

									$this->page_m->update_page_meta($this->_table_meta,$page_meta,array('id'=>(int)$input['flexible_id'][$i]));
								} else {
									$page_meta = array(
										'page_id'			=> $id,
										'item_title'		=> $input['item_title'][$i],
	...
	function alter_table() {
    	$sql = array(
    		...
    		"ALTER TABLE `default_pages` ADD `show_ontab` INT NULL DEFAULT '0' AFTER `order`",
    	);
    ...
    function delete_meta() {
    	...
    	if (!empty($id)) {
    		if ($this->page_m->delete_page_meta($this->_table_meta, $id)) {
    			$json['status']=true;
			} else {
				$json['status']=false;
				$json['message']=lang('pages:delete_error');
			}
		}
