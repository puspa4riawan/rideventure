(function($) {
	$(function() {
		...
		// set values for max.sort_tree (we'll use them below also)
		$item_list	= $('ul.sortable');
		$url		= ADMIN_URL+'/pages/order';
		$cookie		= 'open_pages';
		...
		$item_list.find('li a').live('click', function(e) {
			...
			$details.load(SITE_URL + ADMIN_URL+'/pages/ajax_page_details/' + page_id, function(){
				tmlft();
		...
		post_sort_callback = function() {
			...
			if($details_id.val() > 0)
			{
				$details.load(SITE_URL + ADMIN_URL+'/pages/ajax_page_details/' + $details_id.val());
			}
		}

		// And off we go
		max.sort_tree($item_list, $url, $cookie, data_callback, post_sort_callback);
