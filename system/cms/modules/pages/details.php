 ...
 * @package MaxCMS\Core\Modules\Pages
 */
		...
		if (class_exists('Admin_controller') and $this->db->table_exists('page_types'))
		{
			...
			if ($this->db->count_all('page_types') > 1)
			{
				...
				$info['sections']['pages']['shortcuts'] = array(
					array(
					    ...
					    'uri' => ADMIN_URL.'/pages/choose_type?modal=true',
					    'class' => 'add modal'
					)
				);
			}
			else
			{
				...
				$info['sections']['pages']['shortcuts'] = array(
					array(
					    'name' => 'pages:create_title',
					    'uri' => ADMIN_URL.'/pages/create?page_type='.$page_type->id,
					    'class' => 'add'
					)
				);			
			}
			...
			if ($this->uri->segment(4) == 'fields' and $this->uri->segment(5))
			{
				$info['sections']['types']['shortcuts'] = array(
								array(
								    ...
								    'uri' => ADMIN_URL.'/pages/types/fields/'.$this->uri->segment(5).'/new_field',
								    'class' => 'add'
								)
						    );
			}
			else
			{
				$info['sections']['types']['shortcuts'] = array(
								array(
								    ...
								    'uri' => ADMIN_URL.'/pages/types/create',
								    'class' => 'add'
								)
							);			
			}
