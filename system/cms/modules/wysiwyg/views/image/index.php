<div id="upload-box">
	...
	<?php echo form_open_multipart(ADMIN_URL.'/wysiwyg/upload') ?>
			...
			<table class="table-list" border="0" cellspacing="0">
				...
				<tbody>
					...
					<tr class="<?php echo alternator('', 'alt') ?>">
                                                <td class="image"><img class="max-image" src="<?php echo base_url($image->location === 'local' ? 'files/thumb/'.$image->id.'/50/50' : 'files/cloud_thumb/'.$image->id) ?>" alt="<?php echo $image->alt_attribute ?>" width="50" onclick="javascript:insertImage('<?php echo $image->id."', '".htmlentities(addslashes($image->alt_attribute))."', '".$image->location."', '".$image->path ?>');" /></td>
