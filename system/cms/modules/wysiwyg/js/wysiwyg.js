...
function insertImage(file, alt, location, path)
{
	...
	window.parent.instance.insertHtml('<img class="max-image alignment-'+img_float+'" '+width_tag+' '+float_tag+' src="'+path+'" alt="' + alt + '" />');
    windowClose();
}
...
function insertFile(id, title, location, path)
{
	...
	window.parent.instance.insertHtml('<a class="max-file" href="'+path+'">' + title + '</a>');
    windowClose();
}
...
$(function()
{
	function detectFile()
	{
		...
		if( ! element.hasClass('max-image')) return false;

		$('#current_document').load(SITE_URL + ADMIN_URL+'/wysiwyg/files/ajax_get_file', {
    ...
    $('select[name=parent_id]').live('change', function() {
        ...
        var href_val = SITE_URL + ADMIN_URL+'/wysiwyg/' + controller + '/index/' + folder_id;
        $('#files_right_pane').load(href_val + ' #files-wrapper', function() {
			$(this).children().fadeIn('slow');
