...
/**
 * MaxStreams Row Model
 *
 * @package		MaxStreams
 * @author		MaxCMS Dev Team
 * @copyright	Copyright (c) 2011 - 2013, MaxCMS
 */
class Row_m extends MY_Model {
	...
	public function get_rows($params, $field = null, $stream)
	{
		...
		{
			$this->sql['select'][] = 'DAY('.$this->format_mysql_date($date_by, $stream->stream_namespace).') as maxstreams_cal_day';
		}
		...
		if (isset($paginate) and $paginate == 'yes')
		{
			...
			if ($cache_query)
			{
				...
				$pag_count = $this->maxcache->get($cache_folder.DIRECTORY_SEPARATOR.$cache_hash.'-count');

				if ($pag_count == false) {
	
					$pag_count = $this->db->query($count_sql)->row()->count;
					$this->maxcache->write($pag_count, $cache_folder.DIRECTORY_SEPARATOR.$cache_hash.'-count', $cache_expires);
				}
			}
			else
			{
		...
		if ($cache_query)
		{
			...
			$rows = $this->maxcache->get($cache_folder.DIRECTORY_SEPARATOR.$cache_hash);

			if ($rows == false)
			{
				$rows = $this->db->query($sql)->result_array();
				$this->maxcache->write($rows, $cache_folder.DIRECTORY_SEPARATOR.$cache_hash, $cache_expires);
			}
		}
		else
		{
	...
	{		
		$obj = $this->db->query('
			SELECT '.MAXSTREAMS_DB_PRE.STREAMS_TABLE.'.*, '.MAXSTREAMS_DB_PRE.STREAMS_TABLE.'.id as stream_id, '.MAXSTREAMS_DB_PRE.FIELDS_TABLE.'.* 
			FROM '.MAXSTREAMS_DB_PRE.STREAMS_TABLE.', '.MAXSTREAMS_DB_PRE.ASSIGN_TABLE.', '.MAXSTREAMS_DB_PRE.FIELDS_TABLE.'
			WHERE '.MAXSTREAMS_DB_PRE.STREAMS_TABLE.'.id='.MAXSTREAMS_DB_PRE.ASSIGN_TABLE.'.stream_id and
			'.MAXSTREAMS_DB_PRE.FIELDS_TABLE.'.id='.MAXSTREAMS_DB_PRE.ASSIGN_TABLE.'.field_id');
