...
/**
 * MaxStreams Multiple Relationships Field Type
 *
 * @package		MaxStreams
 * @author		Adam Fairholm
 * @copyright	Copyright (c) 2011 - 2013, Adam Fairholm
 * @link		https://github.com/adamfairholm/MaxStreams-Multiple-Relationships
 */
class Field_multiple
{
	...
	public function alt_pre_output($row_id, $extra, $type, $stream)
	{
		...
		foreach ($query->result() as $node) {
			$html .= '<li><a href="'.site_url(ADMIN_URL.'/streams/entries/view/'.$join_stream->id.'/'.$node->id).'">'.$node->$title_column.'</a></li>';
		}
