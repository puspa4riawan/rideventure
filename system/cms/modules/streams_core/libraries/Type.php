...
/**
 * MaxStreams Core Field Type Library
 *
 * @package		MaxStreams
...
class Type
{
    ...
    {    
		...
		(CMS_VERSION < 1.3) ? define('MAXSTREAMS_DB_PRE', '') : define('MAXSTREAMS_DB_PRE', SITE_REF.'_');
		
		// Since this is MaxStreams core we know where
		// MaxStreams is, but we set this for backwards
		...
		if(defined('MAXPATH'))
		{
			define('MAXSTEAMS_DIR', MAXPATH.'modules/streams_core/');
		}
		else
		{
			define('MAXSTEAMS_DIR', APPPATH.'modules/streams_core/');
		}

		// Set our addon paths
		$this->addon_paths = array(
			'core' 			=> MAXSTEAMS_DIR.'field_types/',
	...
	public function add_css($field_type, $file)
	{
		...
		$html = '<link href="'.site_url('system/cms/modules/streams_core/field_types/'.$field_type.'/css/'.$file).'" type="text/css" rel="stylesheet" />';
	...
	public function add_js($field_type, $file)
	{
		...
		$html = '<script type="text/javascript" src="'.site_url('system/cms/modules/streams_core/field_types/'.$field_type.'/js/'.$file).'"></script>';
