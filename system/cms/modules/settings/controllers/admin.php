 ...
 * @package 	MaxCMS\Core\Modules\Settings\Controllers
 */
class Admin extends Admin_Controller {
	...
	public function edit()
	{
		...
		foreach ($settings as $setting)
		{
			...
			if($setting->slug == 'admin_url')
			{
				$NEW_ADMIN_URL = $setting->value;
			}
		}
		...
		if ($this->form_validation->run())
		{
			...
			foreach ($settings as $setting)
			{
				...
				if($setting->slug == 'admin_url')
				{
					if($this->input->post($setting->slug, false) == 'admin')
					{
						$NEW_ADMIN_URL = $setting->default;
						$new_value = $NEW_ADMIN_URL;
					}
					else
					{
						$NEW_ADMIN_URL = preg_replace("/[^a-zA-Z0-9_-]/", "", $this->input->post($setting->slug, false) );
		...
		redirect( (($NEW_ADMIN_URL != ADMIN_URL)? $NEW_ADMIN_URL: ADMIN_URL).'/settings');
	}
