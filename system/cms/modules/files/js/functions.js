jQuery(function($){
	...
	$('input#file-search').keyup(function(e){
		...
		if (e.which === 13) {
			 ...
			 $.post(SITE_URL+ADMIN_URL+'/files/search', $.extend(tokens,{ search : $(e.target).val() }), function(data){
				tmlft();
					...
					$.each(results.data, function(type, item){
						if (item.length > 0){
							$.each(item, function(i, result){
								 $('<li>'+
										 '<div class="'+type+'"></div>'+
										 '<a data-parent="'+(type === 'folder' ? result.parent_id : result.folder_id)+'" href="'+SITE_URL+ADMIN_URL+'/files#">'+result.name+'</a>'+
	...
	$folders_center.on('dblclick', '.folder', function(e){
		...
		max.files.$last_r_click = $(e.target).closest('li');
	...
	$('ul#folders-sidebar').on('click', '.folder', function(e){
		...
		if ($clicked.is('a')) {
			// store element so it can be accessed the same as if it was right clicked
			max.files.$last_r_click = $clicked.parent('li');
			$('.context-menu-source [data-menu="open"]').trigger('click');
		} else {
			$clicked
	...
	$('#file-breadcrumbs').on('click', 'a', function(e){
		...
		// store element so it can be accessed the same as if it was right clicked
		max.files.$last_r_click = $(e.target);
		$('.context-menu-source [data-menu="open"]').trigger('click');
	});
		...
		$menu_sources.find('li')
			...
			.filter(function(index){
				...
				if ($target.hasClass('file') || $target.is('img')){
					...
					folder = true;
				} else if ($target.hasClass('pane') && max.files.current_level == '0'){
				...
				if ( $(this).attr('data-role') && max.files.has_permissions( $(this).attr('data-role') ) === false ) {
					$(this).hide();
				}
				...
				if (folder && $(this).attr('data-role') === 'synchronize') {
					...
					$item = $(window).data('folder_'+max.files.$last_r_click.attr('data-id'));
	...
	$('.context-menu-source, .button-menu-source').on('click', '[data-menu]', function(e){
		...
		switch (menu){
			case 'open':
				max.files.folder_contents( max.files.$last_r_click.attr('data-id') );
			break;
			...
			case 'new-folder':
				...
				setTimeout(function(){
					max.files.new_folder(max.files.current_level);
				}, 150);
			break;
			...
			case 'download':
				$item = $(window).data('file_'+max.files.$last_r_click.attr('data-id'));
			...
			case 'synchronize':
				max.files.synchronize();
			break;

			case 'delete':
				if ( ! confirm(max.lang.dialog_message)) {
					return;
				}
				max.files.delete_item(max.files.current_level);
			...
			case 'refresh':
				max.files.folder_contents( max.files.current_level );
			break;
		}
	});
	...
	$folders_center.sortable({
		...
		update: function(e) {
			...
			$.post(SITE_URL + ADMIN_URL+'/files/order', $.extend(tokens,{ order : order }), function(data){
				tmlft();
				var results = $.parseJSON(data),
					after_id, moved;
				if (results.status) {
	...
	$(window).on('open-upload', function(){
		...
		if ( ! max.files.$last_r_click.hasClass('file') && max.files.$last_r_click.attr('data-id') > '') {
			max.files.upload_to = max.files.$last_r_click.attr('data-id');
		} else {
			max.files.upload_to = max.files.current_level;
		}

		folder = $(window).data('folder_'+max.files.upload_to);
		...
		$.colorbox({
			...
			onCleanup : function(){
				...
				if (max.files.upload_to === max.files.current_level) {
					max.files.folder_contents(max.files.upload_to);
				}
			...
			onLoad : function(){
				...
				if( ! 'mode' in max.files || max.files.mode != 'replace' ) {
					return;
				}

				// the last thing that has been right clicked should be a file, so this might not be neccessary
				if ( max.files.$last_r_click.hasClass('file') )
				{
					$("#file-to-replace").find('span.name').text(max.files.$last_r_click.attr('data-name')).end().show();
		...
		$form.find('form').fileUploadUI({
			...
			buildUploadRow	: function(files, index, handler){
				...
				if (type && type.search('image') >= 0) {
					resize = 	'<label id="width">'+max.lang.width+'</label>'+
								'<select name="width" class="skip"><option value="0">'+max.lang.full_size+'</option><option value="100">100px</option><option value="200">200px</option><option value="300">300px</option><option value="400">400px</option><option value="500">500px</option><option value="600">600px</option><option value="700">700px</option><option value="800">800px</option><option value="900">900px</option><option value="1000">1000px</option><option value="1100">1100px</option><option value="1200">1200px</option><option value="1300">1300px</option><option value="1400">1400px</option><option value="1500">1500px</option><option value="1600">1600px</option><option value="1700">1700px</option><option value="1800">1800px</option><option value="1900">1900px</option><option value="2000">2000px</option></select>'+
								'<label id="height">'+max.lang.height+'</label>'+
								'<select name="height" class="skip"><option value="0">'+max.lang.full_size+'</option><option value="100">100px</option><option value="200">200px</option><option value="300">300px</option><option value="400">400px</option><option value="500">500px</option><option value="600">600px</option><option value="700">700px</option><option value="800">800px</option><option value="900">900px</option><option value="1000">1000px</option><option value="1100">1100px</option><option value="1200">1200px</option><option value="1300">1300px</option><option value="1400">1400px</option><option value="1500">1500px</option><option value="1600">1600px</option><option value="1700">1700px</option><option value="1800">1800px</option><option value="1900">1900px</option><option value="2000">2000px</option></select>'+
								'<label id="ratio">'+max.lang.ratio+'</label>'+
								'<input name="ratio" type="checkbox" value="1" checked="checked"/>'+
								'<label id="alt">'+max.lang.alt_attribute+'</label>'+
				...
				return $('<li>'+
							...
							'<div class="file_upload_cancel">' +
								'<div title="'+max.lang.start+'" class="start-icon ui-helper-hidden-accessible"></div>'+
								'<div title="'+max.lang.cancel+'" class="cancel-icon"></div>' +
							'</div>' +
			...
			beforeSend: function(event, files, index, xhr, handler, callBack){
				...
				if (files[index].size > max.files.max_size_possible) {
					$progress_div.html(max.lang.exceeds_server_setting);
					return false;
				} else if (files[index].size > max.files.max_size_allowed) {
					$progress_div.html(max.lang.exceeds_allowed);
					return false;
				}

				// is it an allowed type?
				regexp = new RegExp('\\.('+max.files.valid_extensions+')$', 'i');
				...
				if (!regexp.test(files[index].name.toLowerCase())) {
					$progress_div.html(max.lang.file_type_not_allowed);
					return false;
				}

				handler.uploadRow.find('div.start-icon').on('click', function() {
					handler.formData = $.extend(tokens,{
						...
						folder_id: max.files.upload_to,
						replace_id: 'mode' in max.files && max.files.mode == 'replace' ? max.files.$last_r_click.attr('data-id') : 0,
	...
	max.init_upload($('#files-uploader'));


	/***************************************************************************
	 * All functions that are part of the max.files namespace                 *
	 ***************************************************************************/

	max.files.new_folder = function(parent, name) {
		var new_class = Math.floor(Math.random() * 1000),
			post_data;

		if (typeof(name) === 'undefined') {
			name = max.lang.new_folder_name;
		}
		...
		$.post(SITE_URL + ADMIN_URL+'/files/new_folder', $.extend(tokens,post_data), function(data){
			tmlft();
			var results = $.parseJSON(data);
			...
			if (results.status) {
				...
				max.files.$last_r_click = $('.folder[data-id="'+results.data.id+'"]');
	...
	max.files.folder_contents = function(folder_id) {

		var level = max.files.current_level,
			folders = [],
			files = [],
			post_data,
			i = 0,
		...
		$(window).trigger('show-message', {message: max.lang.fetching});

		post_data = { parent : folder_id };
		$.post(SITE_URL + ADMIN_URL+'/files/folder_contents', $.extend(tokens,post_data), function(data){
			tmlft();
			var results = $.parseJSON(data);
			...
			if (results.status) {
				...
				max.files.current_level = folder_id;
				...
				// and we succeeded
				results.message = max.lang.fetch_completed;
				$(window).trigger('show-message', results);
	...
	max.files.rename = function() {

		// what type of item are we renaming?
		var type = max.files.$last_r_click.hasClass('folder') ? 'folder' : 'file',
			$item = max.files.$last_r_click.find('.name-text');
		...
		$input.blur(function(){
			...
			if ( $.isEmptyObject(item_data) || item_data.name !== post['name']) {
				...
				$.post(SITE_URL + ADMIN_URL+'/files/rename_'+type, $.extend(tokens,post), function(data){
	...
	max.files.replace = function() {
		max.files.mode = 'replace';

		$(window).trigger('open-upload');
	};

	max.files.delete_item = function(current_level) {
		...
		items.each(function (index, item) {
			...
			$.post(SITE_URL + ADMIN_URL+'/files/delete_' + type, $.extend(tokens,post_data), function (data) {
				var results = $.parseJSON(data);
				...
				if (results.status) {
					...
					if ($folders_center.find('li').length === 0 && max.files.current_level === 0) {
						$('.no_data').fadeIn('fast');
					}
				}
				$(window).trigger('show-message', results);
	...
	max.files.details = function() {

		var timer, location,
			// file or folder?
			type = max.files.$last_r_click.hasClass('file') ? 'file' : 'folder',
			// figure out the ID from the last clicked item
			$item_id = max.files.$last_r_click.attr('data-id').length > 0 ? max.files.$last_r_click.attr('data-id') : max.files.current_level,
		...
		if ($item) {
		 	...
		 	if ($item.path) { 			$item_details.find('.path')				.val(SITE_URL+'uploads/default/files/'+$item.filename).parent().show(); }
			...
			if (type === 'folder' && $item.file_count === 0 && max.files.has_permissions('set_location') ){
			...
			$('.container-button').on('click', function(e){
				...
				$.post(SITE_URL + ADMIN_URL+'/files/check_container', $.extend(tokens,post_data), function(data) {
					var results = $.parseJSON(data);
					$(window).trigger('show-message', results);
				});
			});
			...
			$item_details.find('.buttons').on('click', 'button', function() {
				if (type === 'file'){
					max.files.save_description($item);
				} else {
					max.files.save_location($item);
				}
				$.colorbox.close();
			});
		}
	...
	max.files.save_description = function(item) {
		...
		if (item.description !== new_description || item.keywords !== new_keywords || item.alt_attribute !== new_alt_attribute){
			...
			$.post(SITE_URL + ADMIN_URL+'/files/save_description', $.extend(tokens,post_data), function(data){
	...
	max.files.save_location = function(item) {
		...
		$.post(SITE_URL + ADMIN_URL+'/files/save_location', $.extend(tokens,post_data), function(data) {
	...
	max.files.synchronize = function() {
		var folder_id = max.files.$last_r_click.attr('data-id'),
			post_data = { folder_id: folder_id };

		$(window).trigger('show-message', { status : null, message : max.lang.synchronization_started });

		$.post(SITE_URL + ADMIN_URL+'/files/synchronize', $.extend(tokens,post_data), function(data){
			tmlft();
			var results = $.parseJSON(data);
			$(window).trigger('show-message', results);
			if (results.status) {
				max.files.folder_contents(folder_id);
			}
		});

	};

	max.files.has_permissions = function(roles) {
		...
		{
			if( $.inArray(actions[x], max.files.permissions) === -1 )
			{
				return false;
			}
		}
	...
	if ($('.folders-center').find('.no_data').length === 0) {
		if (window.location.hash) {
			max.files.folder_contents(window.location.hash);
		} else {
			// deprecated
			max.files.folder_contents(max.files.initial_folder_contents);
		}
	}
});
