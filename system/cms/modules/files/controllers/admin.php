...
/**
 * MaxCMS file Admin Controller
 *
 * Provides an admin for the file module.
 *
 * @author		Jerel Unruh - MaxCMS Dev Team
 * @package		MaxCMS\Core\Modules\Files\Controllers
 */
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->template->append_metadata(
			"<script>
				max.lang.fetching = '".lang('files:fetching')."';
				max.lang.fetch_completed = '".lang('files:fetch_completed')."';
				max.lang.start = '".lang('files:start')."';
				max.lang.width = '".lang('files:width')."';
				max.lang.height = '".lang('files:height')."';
				max.lang.ratio = '".lang('files:ratio')."';
				max.lang.full_size = '".lang('files:full_size')."';
				max.lang.cancel = '".lang('buttons:cancel')."';
				max.lang.synchronization_started = '".lang('files:synchronization_started')."';
				max.lang.untitled_folder = '".lang('files:untitled_folder')."';
				max.lang.exceeds_server_setting = '".lang('files:exceeds_server_setting')."';
				max.lang.exceeds_allowed = '".lang('files:exceeds_allowed')."';
				max.files = { permissions : ".json_encode(Files::allowed_actions())." };
				max.files.max_size_possible = '".Files::$max_size_possible."';
				max.files.max_size_allowed = '".Files::$max_size_allowed."';
				max.files.valid_extensions = '".implode('|', $allowed_extensions)."';
				max.lang.file_type_not_allowed = '".addslashes(lang('files:file_type_not_allowed'))."';
				max.lang.new_folder_name = '".addslashes(lang('files:new_folder_name'))."';
				max.lang.alt_attribute = '".addslashes(lang('files:alt_attribute'))."';

				// deprecated
				max.files.initial_folder_contents = ".(int)$this->session->flashdata('initial_folder_contents').";
			</script>");
	}
	...
	public function initial_folder_contents($id)
	{
		...
		redirect(site_url(ADMIN_URL.'/files'));
	}
