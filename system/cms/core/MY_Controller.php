...
class MY_Controller extends MX_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->library('maxcache');

		// Add the site specific theme folder
		$this->template->add_theme_location(ADDONPATH.'themes/');

		// Migration logic helps to make sure MaxCMS is running the latest changes
		...
		{
			log_message('debug', 'MaxCMS was migrated to version: ' . $schema_version);
		}
		...
		$max['lang'] = $langs[((is_a($this, 'Admin_Controller') )? 'en' : CURRENT_LANGUAGE )];
		$max['lang']['code'] = ((is_a($this, 'Admin_Controller') )? 'en' : CURRENT_LANGUAGE );
