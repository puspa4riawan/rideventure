...
/**
 * This is the basis for the Admin class that is used throughout MaxCMS.
 * 
...
class Admin_Controller extends MY_Controller {
	...
	public function __construct()
	{
		...
		ci()->theme_options = $this->maxcache->model('theme_m', 'get_values_by', array(array('theme' => ADMIN_THEME)));
		...
		if (is_logged_in())
		{
			...
			{				
				...
				if ($module['menu'] and (isset($this->permissions[$module['slug']]) or $this->current_user->group == 'admin'))
				{
					...
					$menu_items['lang:cp:nav_'.$module['menu']][$module['name']] = ADMIN_URL.'/'.$module['slug'];
				}
			...
			$menu_items['lang:global:profile'] = array(
				...
				'lang:cp:logout_label'				=> ADMIN_URL.'/logout'
			);
			...
			unset($ordered_menu['Users']['Permissions']); 
			unset($ordered_menu['Add-ons']['lang:global:plugins']); 
			unset($ordered_menu["Users"] ["Groups"]);
			unset($ordered_menu["Data"] ["Maintenance"]);

			//var_dump($ordered_menu);die();
			// And there we go! These are the admin menu items.
			if($this->current_user->group=='logger'){
				$menu_baru = array(
					'Login Log' => ADMIN_URL.'/users/login_log',
					'Register Log' => ADMIN_URL.'/users/register_log',
					'Logout' => ADMIN_URL.'/logout',
				);
				$this->template->menu_items = $menu_baru;
	...
	private function _check_access()
	{
		...
		$ignored_pages = array(ADMIN_URL.'/login', ADMIN_URL.'/logout', ADMIN_URL.'/help');
		...
		if ($this->current_user->group === 'admin' || $this->current_user->group === 'logger')
		{
			return true;
		}
		...
		if ($this->current_user)
		{
			...
			if ($current_page == ADMIN_URL.'/index' && $this->permissions)
			{
				return true;
			}

            //if we are trying to search the admin area
            if ($current_page == ADMIN_URL.'/search' && $this->permissions)
            {
                return true;
            }
