...
function maximage_onclick(e)
{
    update_instance();
    // run when max button is clicked]
    CKEDITOR.currentInstance.openDialog('maximage_dialog');
}
...
CKEDITOR.plugins.add('maximages',
{
    requires: ['iframedialog'],
    init : function(editor)
    {
        CKEDITOR.dialog.addIframe('maximage_dialog', 'Image', SITE_URL + ADMIN_URL+'/wysiwyg/image',800,500,function(){}, {
			onLoad: function(){
				var id = '#'+this.parts.contents.getId();
				$('.cke_dialog_page_contents', id).css({height:'100%'});
			}
		});
        editor.addCommand('maximages', {exec:maximage_onclick});
        editor.ui.addButton('maximages',{ label:'Upload or insert images from library', command:'maximages', icon:this.path+'images/icon.png' });
		...
		editor.on('selectionChange', function(evt)
		{
			 ...
			 */
			var command = editor.getCommand('maximages'),
				element = evt.data.path.lastElement.getAscendant('img', true);

			// If nothing or a valid document
			if ( ! element || (element.getName() == 'img' && element.hasClass('max-image')))
			{
		...
		editor.on('dialogShow', function(e) {
			...
			if (image && image.hasAttribute('data-maximage')) {
			...
			elements:
			{
				'img': function(element)
				{
					...
					if (unprotectedSrc.indexOf('\{\{') != -1) {
						element.attributes['data-maximage'] = true;
					}
