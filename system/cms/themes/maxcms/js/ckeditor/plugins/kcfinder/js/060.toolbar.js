...
_.uploadFile = function(form) {
    ...
    // CUSTOM BARBAR. because iframe is set to DENY
    var formData = new FormData();    
    var index = 0;
    formData.append('dir', $('input[name="dir"]').val());
    formData.append("upload["+index+"]", $('input[name="upload[]"]')[0].files[0]);
    //var baseURl = window.location.origin + '/max/parentingclub/';
    var baseURl = window.location.origin + '/';
    $.ajax({
        url: baseURl + 'system/cms/themes/maxcms/js/ckeditor/plugins/kcfinder/browse.php?type=images&lng=en&opener=ckeditor&act=upload',
        type: 'POST',
        data: formData,
        contentType: false,
        processData:false,
        ...
        success: function(result, status, xhr){    
            ...
            $('#toolbar a[href="kcact:refresh"]').trigger('click');
        },
        error: function(xhr, ajaxOptions, thrownError){
          ...
          alert('Please contact Developer. ' + thrownError + ': ' + spesificError);
          $('#toolbar a[href="kcact:refresh"]').trigger('click');
        }
    });
    return;
