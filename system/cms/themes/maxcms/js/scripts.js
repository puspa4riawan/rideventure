 ...
 * Max object
 *
 * The Max object is the foundation of all MaxUI enhancements
 */
...
jQuery(function($) {
	...
	// Is Mobile?
	max.is_mobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent);
	...
	$.ajaxSetup({
		...
		data: {
			csrf_hash_name: $.cookie(max.csrf_cookie_name)
		}
	});
	 ...
	 */
	max.init_ckeditor_maximize = function() {
			...
			$.each(CKEDITOR.instances, function(instance) {
				CKEDITOR.instances[instance].on('maximize', function(e) {
					if(e.data == 1) //maximize
					{
						...
						$('.cke_button__maximize').addClass('ckeditor-max-logo');
					}
					else if(e.data == 2) //snap back
					{
						$('.hide-on-ckeditor-maximize').removeClass('hidden');
						$('.cke_button__maximize').removeClass('ckeditor-max-logo');
					}
				});
			});
		}
	 ...
	 * Autocomplete Search
	 */
	max.init_autocomplete_search = function(){
		var cache = {}, lastXhr;
		$(".search-query").autocomplete({
			...
			source: function( request, response ) {
				...
				lastXhr = $.getJSON(SITE_URL + ADMIN_URL+'/search/ajax_autocomplete', request, function( data, status, xhr ) {
	...
	max.init = function() {
		...
		$('a.confirm').live('click', function(e){
			...
			if (confirm(removemsg || max.lang.dialog_message))
			{
		...
		$('a[rel=modal], a.modal').livequery(function() {
			$(this).colorbox({
				...
				onComplete: function(){ max.chosen() }
			});
		});
	...
	max.clear_notifications = function()
	{
		$('.alert .close').click();
	...
	max.add_notification = function(notification, options, callback)
	{
		var defaults = {
			clear	: true,
			ref		: '#content-body',
		...
		opt.clear && max.clear_notifications();
	...
	max.sort_tree = function($item_list, $url, $cookie, data_callback, post_sort_callback, sortable_opts)
	{
		...
		var default_opts = {
			...
			update: function(event, ui) {
				...
				$.post(SITE_URL + $url, $.extend(tokens,post) );
			}
		};
	...
	max.generate_slug = function(input_form, output_form, space_character, disallow_dashes)
	{
	...
	$(document).ajaxError(function(e, jqxhr, settings, exception) {
		if (exception != 'abort' && exception.length > 0) {
			max.add_notification($('<div class="alert error">'+exception+'</div>'));
		}
	});
