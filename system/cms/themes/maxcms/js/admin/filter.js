$(function(){
	...
	max.filter = {
		...
		init: function(){
			...
			$('input[name="f_date_start"]', max.filter.$filter_form).on('datepicking', $.debounce(500, function(){
			...
			$('input[name="f_date_end"]', max.filter.$filter_form).on('datepicking', $.debounce(500, function(){
		...
		do_filter: function(module, form_data, url){
			...
			max.filter.$content.fadeOut('fast', function(){
				...
				$.post(post_url, form_data, function(data, response, xhr) {
					...
					//success stuff here
					max.chosen();
					max.filter.$content.html(html).fadeIn('fast');
				});
			});
		},
