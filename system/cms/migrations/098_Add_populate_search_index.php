		...
		foreach ($this->db->get('pages')->result() as $page)
		{
	    	...
	    	if ($page->status === 'live')
	    	{
	    		...
	    		$this->search_index_m->index(
	    			...
	    			array(
	    				'cp_edit_uri' 	=> ADMIN_URL.'/pages/edit/'.$page->id,
	    				'cp_delete_uri' => ADMIN_URL.'/pages/delete/'.$page->id,
		...
		foreach ($this->db->get('blog')->result() as $post)
		{
	    	...
	    	if ($post->status === 'live')
	    	{
	    		$this->search_index_m->index(
	    			...
	    			$post->intro, 
	    			array(
	    				'cp_edit_uri' 	=> ADMIN_URL.'/blog/edit/'.$post->id,
	    				'cp_delete_uri' => ADMIN_URL.'/blog/delete/'.$post->id,
