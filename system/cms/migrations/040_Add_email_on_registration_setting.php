...
class Migration_Add_email_on_registration_setting extends CI_Migration {
	...
	public function up()
	{
		...
		$this->db->insert('email_templates',array(
			...
			'subject'			=> 'You have just received a registration from {max:name}',
			'body'				=> '<h3>You have received a registration from {max:name}</h3><strong>IP Address: {max:sender_ip}</strong>
<strong>Operating System: {max:sender_os}
<strong>User Agent: {max:sender_agent}</strong>',
			'lang'				=> 'en',
			'is_default'	=> 1
		));
	}
