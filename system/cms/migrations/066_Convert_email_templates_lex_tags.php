...
class Migration_Convert_email_templates_lex_tags extends CI_Migration {
	...
	{	
		...
		{
			$new_subject = preg_replace('/{(\/)?(max:)?([^}]+)}/', '{{ $1$3 }}', $template->subject);
			$new_body = preg_replace('/{(\/)?(max:)?([^}]+)}/', '{{ $1$3 }}', $template->body);
	...
	public function down()
	{
		...
		{
			$new_subject = preg_replace('/{{/s?([^}]+)/s?}}/', '{max:$1}', $template->subject);
			$new_body = preg_replace('/{{/s?([^}]+)/s?}}/', '{max:$1}', $template->body);
