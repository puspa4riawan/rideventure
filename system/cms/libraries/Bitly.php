...
class Bitly {
	...
	public function shorten($long_url)
	{
		...
		if (isset($long_url) AND filter_var($long_url, FILTER_VALIDATE_URL))
		{
			...
			if ($this->_execute('shorten', $params))
			{
				...
				return ($this->_bitly_format != 'json') ? $this->_response : (isset($this->_response['data']['url']) ? $this->_response['data']['url'] : $long_url);
			}
		}
		return FALSE;
	}
	...
	private function _execute($method, $params)
	{
		...
		/*cek database dulu ada apa kagak*/
		$search_data = $this->_ci->db->get_where('default_bitly_cache',array('long_url' => $params['longUrl']));

		if ((bool)$search_data->num_rows()) {
			
			$_response = array(
				'status_code' => 200,
  				'status_txt' => 'OK',
  				'data' => array(
      				'url' => $search_data->row('url'),
      				'hash' => $search_data->row('hash'),
      				'global_hash' => $search_data->row('global_hash'),
      				'long_url' => $search_data->row('url'),
      				'new_hash' => $search_data->row('new_hash')
  					)
				);
		...
		if ($_response = curl_exec($curl))
		{
			...
			if ((bool)count($this->_response['data'])) {	
			$this->_ci->db->insert('default_bitly_cache', $this->_response['data']);
			}
			return TRUE;
		}
		return FALSE;
