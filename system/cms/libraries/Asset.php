 ...
 * 
 * This was a FuelPHP package converted for use within MaxCMS by Phil Sturgeon with 
 * express written consent from Antony Male to be used within MaxCMS Community and Professional
 *
 * @version    v1.11
 * @author     Antony Male
 * @license    MIT License
 ...
 * @package    MaxCMS\Core\Libraries\Asset
 */
...
class Asset {
	...
	*/
	...
	protected static $asset_version = '';
	...
	public function __construct()
	{
		...
		self::$asset_version = $ci->config->item('asset_version');
	}
	...
	public static function get_filepath($filename, $type, $add_url = false, $force_array = false)
	{
		...
		foreach ($files as &$file)
		{
			...
			if ($add_url)
			{
				$file = self::$asset_url . $file . (self::$asset_version!='' ? '?v='.self::$asset_version : '');
			}
		}
			...
			if (self::$groups['js'][$group_name]['combine'])
			{
				...
				else
				{
					...
					$ret .= self::html_tag('script', array(
						'src' => self::$asset_url.$filepath.(self::$asset_version!='' ? '?#v='.self::$asset_version : ''),
					) + $attr, '').PHP_EOL;
				}
			}
			else
			{
				foreach ($file_group as $file)
				{
					...
					else
					{
						...
						$ret .= self::html_tag('script', array(
							'src' => $base.$filepath.(self::$asset_version!='' ? '?#v='.self::$asset_version : ''),
						) + $attr, '').PHP_EOL;
					}
				}
			}
			...
			if (self::$groups['css'][$group_name]['combine'])
			{
				...
				else
				{
					...
					$ret .= self::html_tag('link', array(
						'rel' => 'stylesheet',
						'href' => self::$asset_url.$filepath.(self::$asset_version!='' ? '?#v='.self::$asset_version : ''),
					) + $attr).PHP_EOL;
				}
			}
			else
			{
				foreach ($file_group as $file)
				{
					...
					else
					{
						...
						$ret .= self::html_tag('link', array(
							'rel' => 'stylesheet',
							'href' => $base.$filepath.(self::$asset_version!='' ? '?#v='.self::$asset_version : ''),
						) + $attr).PHP_EOL;
					}
				}
			}
	...
	public static function img($images, $alt, $attr = array())
	{
		...
		foreach ($images as $image)
		{
			...
			foreach ($image_paths as $image_path)
			{
				...
				$base = ($remote) ? '' : self::$asset_url;
				$attr['src'] = $base.$image_path.(self::$asset_version!='' ? '?#v='.self::$asset_version : '');
