 ...
 * @copyright  Copyright (c) 2006 - 2012 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Writer_XML implements PHPExcel_Writer_IWriter {
	/**
	 * PHPExcel object
	 *
	 * @var PHPExcel
	...
	private $char_close = '>';
	private $char_close_backslash = '/';
	
	private $row_header = 1;
	
	private $group_name = '';
	
	private $header_content = array();
	//end tambahan selen
	...
	public function save($pFilename = null) {
		...
		for($row = 1; $row <= $maxRow; ++$row) {
			...
			if($this->row_header == $row)
			{
				//set the header_name
				$this->header_content = $cellsArray[0];
			}
			else {
	...
	public function setCharOpen($char)
	{
		$this->char_open = $char;
	}
	
	public function getCharOpen()
	{
		return $this->char_open;
	}
	
	public function setCharClose($char)
	{
		$this->char_close= $char;
	}
	
	public function getCharClose()
	{
		return $this->char_close;
	} 
	...
	private function _writeLine($pFileHandle = null, $pValues = null) {
		if (is_array($pValues)) {
			...
			// Build the line
			$line = '';
			//$line.= $this->char_open.$this->group_name.$this->char_close;
			foreach ($pValues as $_idx =>$element) {
				// Escape enclosures
				...
				// Add enclosed string
				$line .= $this->char_open. $this->header_content[$_idx] .$this->char_close.$element.$this->char_open.$this->char_close_backslash. $this->header_content[$_idx] .$this->char_close ;
			}
			
			//$line.= $this->char_open.$this->char_close_backslash.$this->group_name.$this->char_close;
